...
**Full DPDCA loop reference:** `.github/DPDCA-WORKFLOW.md`

**Story ID Invariant (mandatory -- agents must not violate)**
- IDs are assigned by `seed-from-plan.py` in sequential PLAN.md parse order
- `reflect-ids.py` writes `[ACA-NN-NNN]` annotations into PLAN.md story lines
- After reflecting, every story line reads: `Story 2.5.4 [ACA-02-017]  title...`
- Sprint manifests are generated by `gen-sprint-manifest.py` -- IDs come from veritas-plan.json
- Agents: read PLAN.md for the `[ACA-NN-NNN]` annotation -- NEVER invent or guess an ID
- Inserting a story in the MIDDLE of an epic shifts all subsequent IDs -- always append at end
