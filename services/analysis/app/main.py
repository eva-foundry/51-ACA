...
try:
    ...
    _CosmosClient = None  # type: ignore
    _DefaultAzureCredential = None  # type: ignore


def _get_cosmos_client():
    """Return a CosmosClient or None if ACA_COSMOS_URL is absent (CI mode)."""
    url = os.environ.get("ACA_COSMOS_URL", "")
    if not url or _CosmosClient is None:
        return None
    return _CosmosClient(url=url, credential=_DefaultAzureCredential())
...
def main() -> int:
    ...
    assembler = FindingsAssembler(scan_id=scan_id, subscription_id=sub_id, cosmos_client=_get_cosmos_client())
